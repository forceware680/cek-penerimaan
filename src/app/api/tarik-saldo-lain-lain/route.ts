...
export async function GET(request: NextRequest) {
    try {
        ...
        ;WITH FilteredData AS (
            SELECT 
                d.NoTerima, d.ObjekPersediaan, op.Keterangan AS NamaBarang,
                d.Satuan, d.MerkType, d.Jumlah, d.Harga,
                (d.Jumlah * d.Harga) AS TotalHarga, h.TglBast AS BAST, h.NoBAST,
                d.Kadaluwarsa, d.Keterangan, 'Saldo Lain-lain' AS TipeSaldo,
                h.TglBast AS TglTransaksi, LEFT(d.NoTerima, 16) AS PBSubkNoDot,
                CASE 
                ...
                d.ObjekPersediaan + '_' + CONVERT(VARCHAR(8), h.TglBast, 112) + '_' +
      ...
      `);
        ...
        console.error('Tarik saldo lain-lain error:', error);
